...
class NullCipher : public BlockCipher {
 public:
  ...
  virtual bool setKey(const CipherKey &) { return true; }

  virtual bool encrypt(const byte *, const byte *in, byte *out, int numBytes) {
    if (in != out) memcpy(out, in, numBytes);
    return true;
  }

  virtual bool decrypt(const byte *, const byte *in, byte *out, int numBytes) {
    if (in != out) memcpy(out, in, numBytes);
    return true;
  }
