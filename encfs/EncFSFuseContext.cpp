...
bool EncFSFuseContext::isMounted()
{
  return (bool) _fs;
}

size_t EncFSFuseContext::openFileCount()
{
  return _openFileCount;
}

void EncFSFuseContext::setRunning(bool running)
{
  _running = running;
}

bool EncFSFuseContext::isRunning()
{
  return _running;
}

bool EncFSFuseContext::isPublic()
{
  return _args->isPublic;
}
...
File EncFSFuseContext::releaseFile(const char *path, struct fuse_file_info *fi)
{
  auto fptr = (File *) (uintptr_t) fi->fh;
  assertPath( path, fptr );
...
void EncFSFuseContext::unmountFS()
{
  _fs = nullptr;
}

std::shared_ptr<EncFS_Args> EncFSFuseContext::getArgs()
{
  return _args;
}
