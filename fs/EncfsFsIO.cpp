...
EncfsDirectoryIO::EncfsDirectoryIO(DirTraverse dt)
: _dt( std::move( dt ) )
{
}
...
: _fnode( std::move( fnode ) )
, _isWritable( std::move( isWritable ) )
{
}

Interface EncfsFileIO::interface() const
{
  return EncfsFileIO_iface;
}
...
bool EncfsFileIO::isWritable() const
{
  return _isWritable;
}
...
std::shared_ptr<DirNode> EncfsFsIO::getRoot() const
{
...
File EncfsFsIO::openfile(const Path &path,
                         bool requestWrite,
                         bool createFile)
{
  int res;
  ...
  return std::unique_ptr<EncfsFileIO>( new EncfsFileIO( std::move( fnode ), requestWrite ) );
}
...
void EncfsFsIO::posix_mknod(const Path &pathSrc, fs_posix_mode_t mode, fs_posix_dev_t dev)
{
...
void EncfsFsIO::posix_setxattr(const Path &path, bool follow,
                               std::string name, size_t offset,
                               std::vector<byte> buf, PosixSetxattrFlags flags)
{
  const int res = getRoot()->posix_setxattr( path.c_str(), follow,
                                            ...
                                            std::string name, size_t offset, size_t amt) const
{
  ...
  const int res = getRoot()->posix_getxattr( &ret,
                                             path.c_str(), follow,
...
PosixXattrList EncfsFsIO::posix_listxattr(const Path &path, bool follow) const
{
  ...
  const int res = getRoot()->posix_listxattr( &ret,
                                              path.c_str(), follow );
  if(res < 0) throw create_errno_system_error( -res );
...
void EncfsFsIO::posix_removexattr(const Path &path, bool follow, std::string name)
{
